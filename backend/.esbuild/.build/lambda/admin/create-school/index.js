...
// lambda/admin/create-school/index.ts
var create_school_exports = {};
__export(create_school_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(create_school_exports);
...
var unauthorizedResponse = (message = "Unauthorized") => {
  return errorResponse(message, 401);
};
var forbiddenResponse = (message = "Forbidden") => {
  return errorResponse(message, 403);
};
var notFoundResponse = (message = "Not found") => {
...
var extractTokenFromHeader = (authHeader) => {
...
var withAuth = (handler2, requiredRoles) => {
  return async (event) => {
    try {
      ...
      return await handler2(authenticatedEvent);
    } catch (error) {
      return unauthorizedResponse(error.message || "Authentication failed");
    }
  };
};
var requireSuperAdmin = (handler2) => {
  return withAuth(handler2, ["SUPER_ADMIN" /* SUPER_ADMIN */]);
};

// lambda/admin/create-school/index.ts
var docClient = getDynamoDBClient();
var tableName = getTableName();
var createSchoolSchema = import_joi.default.object({
  name: import_joi.default.string().min(3).max(100).required(),
  description: import_joi.default.string().min(10).max(500).required(),
  adminId: import_joi.default.string().required(),
  logo: import_joi.default.string().uri().optional(),
  website: import_joi.default.string().uri().optional(),
  status: import_joi.default.string().valid("ACTIVE", "INACTIVE").default("ACTIVE")
});
var handlerLogic = async (event) => {
  try {
    ...
    const currentRoles = admin.roles || ["USER" /* USER */];
    if (!currentRoles.includes("ADMIN" /* ADMIN */)) {
      currentRoles.push("ADMIN" /* ADMIN */);
      await docClient.send(
...
var handler = requireSuperAdmin(handlerLogic);
