...
var revoke_all_exports = {};
__export(revoke_all_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(revoke_all_exports);
...
var revokeAllUserSessions = async (userId) => {
  ...
  if (sessionsResult.Items && sessionsResult.Items.length > 0) {
    const deletePromises = sessionsResult.Items.map(
      (item) => docClient.send(
